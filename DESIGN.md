...
- **Finder context menu**: an `NSServices` provider ("Put on the table") declared in the macOS `Info.plist`, so a right-click on any selection of files in Finder reaches the same intake. The system hands the selection over on an `NSPasteboard` and launches the app if it is not running; the service is `NSSendFileTypes = public.data` for the same reason the document type is — folders are not upload sources. This is the Services menu, not Share: a share extension would put the app behind Finder's Share submenu and cannot upload anyway (see the iOS extension above), while a service is delivered to the running app, which is the only process allowed to hash and transfer.
